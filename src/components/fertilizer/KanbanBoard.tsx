...
export const KanbanBoard = ({ plans, onStatusUpdate }: KanbanBoardProps) => {
  const columns: Column[] = [
    ...
    },
    {
      ...
      title: 'In Progress',
      type: 'in-progress',
      plans: plans.filter(p => p.status === 'in-progress'),
    },
    {
      id: 'complete',
      title: 'Complete',
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    try {
      const updatedPlan = await updatePlanStatus(plan.id, destination.droppableId as any);
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="overflow-x-auto pb-4 -mx-6 px-6">
        <div className="inline-flex md:grid md:grid-cols-3 gap-6 min-w-[calc(100vw-2rem)] md:min-w-0">
          {columns.map((column) => (
            <div key={column.id} className="flex flex-col h-full w-[85vw] md:w-full flex-shrink-0">
